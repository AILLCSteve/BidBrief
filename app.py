...
def generate_question_set():
    ...
    system_prompt = (
        ...
        "CRITICAL — DOCUMENT ANALYSIS QUESTIONS ONLY:\n"
        "Every single question you produce MUST be a document analysis question — a question that an AI "
        "will pose directly to an uploaded document (bid spec, RFP, contract, scope of work, etc.) in order "
        "to extract specific information from it. Questions must be answerable by reading the document.\n\n"
        "NEVER produce:\n"
        "  - Interview questions for a person (e.g. 'How many years of experience do you have?')\n"
        "  - Knowledge-test questions about a topic (e.g. 'What are best practices for X?')\n"
        "  - Opinion or open-ended discussion questions\n\n"
        "ALWAYS produce questions in the form:\n"
        "  - 'What does the document specify regarding [topic]?'\n"
        "  - 'Identify all [requirements/clauses/terms] related to [topic] in this document.'\n"
        "  - 'What [deadlines/values/quantities/requirements] does the document state for [topic]?'\n"
        "  - 'Does the document include [provision/requirement]? If so, what does it say?'\n\n"
        "If the user's input describes a domain, role, or topic (rather than document-specific questions), "
        "convert that context into document-analysis questions — questions whose answers would be found in "
        "a procurement document, specification, or contract for that domain.\n\n"
        "EXAMPLE CONVERSION:\n"
        "  User writes: 'underground utility contractor hiring'\n"
        "  WRONG output: 'How many years of experience do you have in underground utility work?'\n"
        "  CORRECT output: 'What qualifications or experience requirements does the document specify for "
        "the contractor performing underground utility work?'\n\n"
        "RULES:\n"
        "1. Preserve EVERY specific question the user wrote, word-for-word, without abridging or paraphrasing. "
        "   If a user-supplied question is clearly a person-directed question, reframe it as a document-directed "
        "   equivalent while preserving the intent.\n"
