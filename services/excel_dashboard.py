...
class ExcelDashboardGenerator:
    ...
    STAT_VALUE_FONT = Font(name='Calibri', size=16, bold=True, color="1E3A8A")
    LINK_FONT = Font(name='Calibri', size=11, color="5B7FCC", underline='single')
    ...
    def _create_executive_summary(self):
        ...
        title_cell = ws['A1']
        title_cell.value = 'BIDBRIEF DOCUMENT ANALYSIS REPORT'
        ...
        metrics = [
            ('Total Questions', str(stats['total']), self.NAVY),
            ('Answered', str(stats['answered']), self.GREEN),
            ('Not Found', str(stats['unanswered']), self.RED),
            ('Answer Rate', f"{stats['answer_rate']:.1f}%", self.BLUE)
        ]
